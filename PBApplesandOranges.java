...
class Result {
   ...
   long suba=s-a;
   long counta=0,counto=0;
    
   for(int i=0;i<m;i++){
       if(Math.abs(apples.get(i))>=suba && apples.get(i)>0 && apples.get(i)<=(t-a)){
           counta++;
       }   
   }
   long subo=b-t;
   for(int j=0;j<n;j++){
       if(Math.abs(oranges.get(j))>=subo && oranges.get(j)<0 && oranges.get(j)<=(b-s)){
           counto++;
       }
   }
...
public class PBApplesandOranges {
